...
class Wfs {
  ...
  constructor(clientUid,  sessionId, webClientUid, streamAddr, streamPort, callback,config = {}) {

    this.clientUID = clientUid
    this.sessionID = sessionId
    this.webClientUid = webClientUid
    this.streamSize = 0
    this.playStatus = true// true:开始接收
  ...
  addStreamEventCallback(eventNames,callback){
    if(eventNames instanceof  Array){
      for(let idx =0;idx<eventNames.length;idx++){
        const curEventName = eventNames[idx]
        this.media.addEventListener(curEventName,function(){
          callback(curEventName)
        },false);
      }
    }
    else{
      this.media.addEventListener(eventNames,function(){
        callback(eventNames)
      },false);
    }
